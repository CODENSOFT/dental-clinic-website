        ...
        <div className="sm:hidden min-w-0">
          {loading ? (
            <div className="flex items-center justify-center py-8">
              <div className="relative">
                ...
                <div className="absolute inset-0 flex items-center justify-center">
                  <FontAwesomeIcon icon={faTooth} className="text-blue-600 text-sm" />
                </div>
              </div>
            </div>
            ...
            <div className="text-center py-8">
              ...
              <p className="text-sm text-gray-600 font-semibold px-2">Nu sunt ore disponibile pentru această dată</p>
              <p className="text-xs text-gray-500 mt-1 px-2">Te rugăm să alegi o altă dată</p>
            </div>
          ) : (
            <select
              value=""
          ...
          {loading ? (
            <div className="col-span-full flex items-center justify-center py-12 md:py-16">
              <div className="relative">
                ...
                <div className="absolute inset-0 flex items-center justify-center">
                  <FontAwesomeIcon icon={faTooth} className="text-blue-600 text-base md:text-lg" />
                </div>
              </div>
            </div>
          ) : timeSlots.filter(t => !bookedTimes.includes(t)).length === 0 ? (
            <div className="col-span-full text-center py-12 md:py-16">
              ...
              <p className="text-base text-gray-600 font-semibold px-2">Nu sunt ore disponibile pentru această dată</p>
              <p className="text-sm text-gray-500 mt-2 px-2">Te rugăm să alegi o altă dată</p>
            </div>
          ) : (
            timeSlots.filter(t => !bookedTimes.includes(t)).map((t) => (
              <button
...
export default function Home() {
  ...
  const handleNewsletterSubmit = async (e: React.FormEvent) => {
    ...
    if (!newsletterEmail.trim()) {
      setNewsletterError("Te rugăm să introduci o adresă de email validă.");
      return;
    }
    ...
    if (!emailRegex.test(newsletterEmail)) {
      setNewsletterError("Adresa de email nu este validă.");
      return;
    }
    ...
    try {
      ...
      // Here you would make an actual API call to save the email
      ...
      setNewsletterError("A apărut o eroare. Te rugăm să încerci din nou.");
    } finally {
            ...
            <div>
              ...
              <form onSubmit={handleNewsletterSubmit} className="relative group">
                <input
                  ...
                  className="w-full bg-gray-800 border-2 border-gray-700 rounded-[50px] px-4 sm:px-6 pr-16 sm:pr-20 py-3 sm:py-4 text-white placeholder-gray-400 focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 text-sm sm:text-base transition-all duration-200 group-hover:border-gray-600 disabled:opacity-50"
                />
                <button
                  type="submit"
                  disabled={newsletterSubmitting}
                  className="absolute right-[3px] top-[3px] bottom-[3px] aspect-square w-12 h-12 sm:w-14 sm:h-14 bg-gradient-to-r from-blue-600 to-blue-700 text-white rounded-full hover:from-blue-700 hover:to-blue-800 transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 focus:ring-offset-gray-900 flex items-center justify-center shadow-lg hover:shadow-xl hover:scale-105 active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  {newsletterSubmitting ? (
                    ...
                    <svg className="w-4 h-4 sm:w-5 sm:h-5 transform rotate-300" fill="currentColor" viewBox="0 0 24 24">
              ...
              {/* Success Message */}
              {newsletterSuccess && (
                <div className="mt-3 sm:mt-4 p-3 sm:p-4 bg-green-500/20 border-2 border-green-500/50 rounded-xl sm:rounded-2xl text-green-300 text-sm sm:text-base animate-fade-in shadow-lg">
                  <div className="flex items-start">
                    ...
                    <span className="flex-1 leading-relaxed">Mulțumim! Te-ai abonat cu succes la buletinul nostru informativ.</span>
                  </div>
                </div>
              )}
              ...
              {/* Error Message */}
              {newsletterError && (
                <div className="mt-3 sm:mt-4 p-3 sm:p-4 bg-red-500/20 border-2 border-red-500/50 rounded-xl sm:rounded-2xl text-red-300 text-sm sm:text-base animate-fade-in shadow-lg">
                  <div className="flex items-start">
                    <svg className="w-5 h-5 sm:w-6 sm:h-6 mr-2 sm:mr-3 flex-shrink-0 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M6 18L18 6M6 6l12 12" />
                    </svg>
                    <span className="flex-1 leading-relaxed">{newsletterError}</span>
                  </div>
                </div>
              )}
            </div>
